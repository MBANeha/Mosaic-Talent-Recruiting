    ...
    <Page>
      ...
      <section className="mx-auto max-w-4xl px-5 py-14 sm:px-8">
        ...
        {acceptedAwaitingSchedule.map((r) => {
          ...
          if (!pick) return null;
          return (
            <Card key={r.id} className="mt-6 flex gap-5 p-7">
              <PickPhoto pick={pick} blurred variant="thumb" />
        ...
        {upcoming.map((d) => {
          ...
          return (
            <Card key={d.id} className="mt-6 overflow-hidden p-0">
              <div className="flex gap-5 bg-plum p-7 text-cream">
                <PickPhoto pick={pick} blurred variant="thumb" />
                <div>
                  ...
                  <p className="mt-3 text-xs text-cream/40">
                    A calendar invitation has been sent automatically. No contact information is exchanged
                    before the date.
                  </p>
                </div>
              </div>
                ...
                return (
                  <Card key={d.id} className="flex items-center justify-between gap-4 p-5">
                    <div className="flex items-center gap-3">
                      <PickPhoto pick={pick} blurred={!d.mutual} variant="circle" />
                      <div>
